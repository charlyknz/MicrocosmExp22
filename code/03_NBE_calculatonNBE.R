...
for(i in 1:length(USI)){
  ...
  if(dim(temp)[1]>2){#does the next step only if at least 3 data points are present
    ...
    AUC.deltaRR_spline<-auc(temp$sampling, temp$delta_RR,  from = min(temp$sampling, na.rm = TRUE), to = max(temp$sampling, na.rm = TRUE),
    ...
    stab.auc<-rbind(stab.auc,
                    tibble(temp[1,c(1:3,7,8,25)],
                               NBE,
                               AUC.deltaRR,  AUC.deltaRR_spline,                             
                               AUC.delta,
                               AUC.ges.RR,
...
#test linear vs spline relationship 
lm1<-lm(stab.auc$AUC.deltaRR_spline ~ stab.auc$AUC.deltaRR)
summary(lm1)

ggscatter(stab.auc, x = 'AUC.deltaRR',y = 'AUC.deltaRR_spline')
...
#### Expected Stability: 4-species and 5-species Mix ####
  ...
  if(dim(temp)[1]>2){#does the next step only if at least 3 data points are present
    ...
    stab.auc.mix<-rbind(stab.auc.mix,
                        data.frame(temp[1,c(1:3,7,8,24)],
                                   NBE,AUC.deltaRR,AUC.deltaRR_spline,
...
#test linear vs natural splines
lm2<-lm(stab.auc.mix$AUC.deltaRR_spline ~ stab.auc.mix$AUC.deltaRR)
summary(lm2)

ggscatter(stab.auc.mix, x = 'AUC.deltaRR',y = 'AUC.deltaRR_spline')
...
p2 <- d3%>%
  ...
  ggplot(.)+
  geom_hline(yintercept = 0, color = 'darkgrey')+
 # geom_point(aes(x = as.factor(N), y = NBE,color = temp, shape = temp),size = 1, alpha = 0.2)+
  # geom_boxplot(aes(x = as.factor(N), y = AUC.ges.RR, col = temp),size = 1, alpha = 0.8)+
  geom_point(aes(x = as.factor(N), y = mean.total.DRR, color=temp, shape=temp),size = 3.5, alpha = 0.8)+
...
plot_grid( p2,p1+theme(legend.position = 'none'),blank,blank, labels = c('(a)', '(b)'), ncol = 2,rel_widths = c( 1/3,2/3), rel_heights = c(1,0.3))
ggsave(plot = last_plot(), file = here('/Users/charlottekunze/Desktop/Exp22/MicrocosmExp22/output/plot-grid_fig2.jpeg'),width = 12, height = 6)
...
ggsave(plot = last_plot(), file = here('/Users/charlottekunze/Desktop/Exp22/MicrocosmExp22/output/Fig4_ObservedStab_patchwork.png'),width = 10, height = 8)
...
MonoData %>%
  ggplot(.)+
  geom_hline(yintercept = 0, color = 'darkgrey')+
  geom_errorbar(aes(x = N, y = mean,ymin = mean - se, ymax = mean +se, color = temp), width = .3, alpha = .7)+
  geom_point(aes(x = N, y = mean, color = temp, shape = temp),size = 2.5, alpha = .6)+
  labs(x = 'Combination', y = 'Observed Stability', color = 'Treatment', shape = 'Treatment')+
  scale_colour_brewer(palette = "Set1")
...
d1%>%
  ...
  mutate(N = str_length(combination),
         ...
         label = paste(ifelse(N == 1, 'Monoculture', ifelse( N == 2, '2 species', ifelse(N == 4, '4 species', '5 species')))))%>%
  group_by(temp, N)%>%
  mutate(meanstab = mean(AUC.RR_obs, na.rm = T),
         sdstab  = sd(AUC.RR_obs, na.rm = T),
         se  = sdstab/sqrt(n())) %>%
  group_by(temp, N, combination, meanstab, se)%>%
  summarise(raw = mean(AUC.RR_obs, na.rm = T)) %>%
  ggplot(.)+
  geom_hline(yintercept = 0)+
  geom_point(aes(x = N, y = raw, color = temp), alpha = 0.5)+
  facet_grid(~temp)+
  labs(x = 'Species Richness', y = 'Observed Stability',color = 'Treatment')+
  geom_point(aes(x = N, y = meanstab, color = temp), size = 2)+
  geom_errorbar(aes(x = N,ymin = meanstab - se, ymax = meanstab + se, color = temp))+
  scale_colour_manual(values = tempPalette)+
  theme_bw()
...
ggplot(.) +
  ...
  facet_wrap(~temp)+
  labs(x = 'Species Richness', y= 'NBE on Temporal Varability (CV)')+
  theme_bw()+
  theme(legend.position = 'bottom',
        panel.grid.major=element_blank(),panel.grid.minor=element_blank()) + 
  theme(axis.title.x = element_text(size = 14,face = "plain", colour = "black", vjust = 0),
...
NBES_supp <- plot_grid(NBESresistance, NBESCV,ncol = 1, labels = c('(a)','(b)'))
NBES_supp
ggsave(plot = last_plot(), file = here('MicrocosmExp22/output/NBES_metrics.png'), width = 8, height = 8)
